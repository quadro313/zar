...
export default class Notations {
    ...
    //CONSIDER maybe reconsider this?
    set notations(val) {
    ...
    spawnDice(string, amount) {
        ...
        if(string && string.length > 0) {
            ...
            if(numericPattern.test(string) && diceList.find(e => numericPattern.test(e.label))) {
                let sides = string.match(numericPattern)[2];
                let diceConfig = {label: string.match(numericPattern)[0], config: +sides};
                return R.times(() => new Dice(diceConfig), amt);
            }
            else if(found) {
                return R.times(() => new Dice(found), amt);
            }
            else throw new Error("Selected notation contains no such labeled dice.");
            
        }
        else {
            return R.times(() => new Dice(diceList[randomInt(0, diceList.length)]), amt);
        }
    }
}